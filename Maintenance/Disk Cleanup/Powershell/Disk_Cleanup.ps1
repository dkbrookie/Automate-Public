...
If ((Test-Path "$env:windir\System32\cleanmgr.exe" -PathType Leaf)) {
    ...
    While ($diskCleanupRunning -and ($TimeSpan.TotalSeconds -gt 0)) {
        ...
        $diskCleanupRunning = Get-Process $proc -EA 0
...
Write-Output "trashVar=$Null|before=$before|after=$after|spaceSaved=$saved|spaceSavedGBs=$($savedGBs)GBs"