...
If ((Test-Path -Path $ltsvcPath)) {
    ...
    Get-ChildItem -Path $ltsvcPath -Include *.* -File -Recurse | Where-Object { $_.Directory.Name -ne 'PSExec' -and $_.Directory.Name -ne 'Icons' -and $_.Directory.Name -ne 'Ninite' -and $_.LastWriteTime -le (Get-Date).AddDays(-$age) } | Remove-Item -Force -Confirm:$False
}
